...
/** List the values of a mapping. */
public final class ValuesFilter implements NamedFilter {

  public static final ValuesFilter INSTANCE = new ValuesFilter();
  ...
  public Value apply(Value v, List<Value> args, Map<String, Value> kwargs, SourceLocation loc) {
    if (v instanceof Value.MapVal mv) {
      return Value.ofList(new ArrayList<>(mv.entries().values()));
    }
    throw new TemplateException("values() requires a mapping", loc);
  }
}
