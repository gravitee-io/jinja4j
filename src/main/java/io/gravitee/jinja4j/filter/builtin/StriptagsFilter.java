...
public final class StriptagsFilter implements NamedFilter {

  public static final StriptagsFilter INSTANCE = new StriptagsFilter();
  ...
  public Value apply(Value v, List<Value> args, Map<String, Value> kwargs, SourceLocation loc) {
    var s = v.asString().replaceAll("<[^>]*>", "");
    return Value.of(s.replaceAll("\\s+", " ").strip());
  }
}
