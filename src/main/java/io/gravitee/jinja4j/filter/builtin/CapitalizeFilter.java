...
public final class CapitalizeFilter implements NamedFilter {

  public static final CapitalizeFilter INSTANCE = new CapitalizeFilter();

  private CapitalizeFilter() {}
  ...
  public Value apply(Value v, List<Value> args, Map<String, Value> kwargs, SourceLocation loc) {
    var s = v.asString();
    if (s.isEmpty()) return Value.of(s);
    return Value.of(Character.toUpperCase(s.charAt(0)) + s.substring(1).toLowerCase());
  }
}
